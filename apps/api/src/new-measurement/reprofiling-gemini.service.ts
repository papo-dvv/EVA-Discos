...
const ESQUEMA_RESPUESTA = {
  ...
  required: ['tipoFormato', 'trenNumero', 'kilometraje', 'puestoTrabajo', 'fecha', 'horaInicio', 'horaFin', 'confianza', 'filas', 'advertencias'],
  properties: {
    tipoFormato: { type: 'string', enum: ['UT-UF-MTO-FR-414', 'CONCAR', 'otro'] },
...
export class ReprofilingGeminiService {
  ...
  async leer(archivo: Express.Multer.File): Promise<ResultadoOcrReperfilado> {
    ...
    if (!archivo?.buffer?.length) throw new ServiceUnavailableException('No se recibió ninguna fotografía.');
...
El lado izquierdo está a la izquierda del bloque central EJE/RUEDA/COCHE y el derecho a la derecha. Usa el número de EJE impreso en el centro para asignar cada fila. Convierte coma decimal a punto. Fecha en YYYY-MM-DD y horas en HH:mm. Confianza de 0 a 100 por fila. Agrega una advertencia específica por cada zona dudosa o recortada.`;
    ...
    try {
      const respuesta = await ai.models.generateContent({
        model: this.config.get<string>('GEMINI_VISION_MODEL')?.trim() || 'gemini-3.5-flash',
        contents: [{
          role: 'user',
          parts: [
            { text: prompt },
            { inlineData: { data: imagen.toString('base64'), mimeType: 'image/jpeg' } },
          ],
        }],
        config: {
          temperature: 0,
      ...
      const unicas = new Map(datos.filas.map((fila) => [`${fila.ejeNumero}|${fila.lado}`, fila]));
      const advertencias = [...datos.advertencias];
      if (datos.tipoFormato !== 'UT-UF-MTO-FR-414') {
        advertencias.unshift(`La fotografía parece corresponder al formato ${datos.tipoFormato}, no a UT-UF-MTO-FR-414; revisa todos los campos.`);
      }
      return {
      ...
      throw new BadGatewayException(`Gemini no pudo leer la fotografía: ${mensaje}`);
    }
  }
}
