...
const NOMBRE_PLANTILLA = 'UT-UF-MTO-FR-414 CONTROL DE TRABAJOS EN TORNO FOSA_260730_103107.pdf';
...
export class ReprofilingPdfService {
  ...
  async generar(fichaId: string): Promise<Uint8Array> {
    const ficha = await this.prisma.measurementSheet.findUnique({
      where: { id: fichaId },
      include: { instrumentos: { orderBy: { posicion: 'asc' } }, tecnicos: { orderBy: { posicion: 'asc' } } },
    });
    ...
    const porPosicion = new Map(filas.map((fila) => [`${fila.ejeExcel}|${fila.ubicacionExcel}`, fila]));
    ...
    const escribir = (valor: unknown, x: number, y: number, size = 6.4, negrita = false) => {
      const contenido = String(valor ?? '').trim();
      if (contenido) page.drawText(contenido, { x, y, size, font: negrita ? bold : font, color: tinta });
    };
    const numero = (valor: unknown) => valor === null || valor === undefined ? '' : Number(valor).toFixed(1);
    const fecha = (valor: Date | null) => valor ? valor.toLocaleDateString('es-PE', { timeZone: 'America/Lima' }) : '';
    const fechaHora = (valor: Date | null) => valor
    ...
    if (ficha.trenNumero <= 5) {
      page.drawRectangle({ x: 180, y: 704, width: 96, height: 10, color: rgb(1, 1, 1) });
    ...
    const columnas = {
      izqAntesT: 94, izqAntesH: 128, izqDespuesT: 162, izqDespuesH: 196, izqRa: 229,
      derAntesT: 386, derAntesH: 420, derDespuesT: 454, derDespuesH: 488, derRa: 521,
    };
    for (let eje = 1; eje <= 24; eje++) {
      ...
      escribir(numero(izq ? Number(izq.tValue) - Number(izq.hValue) : null), columnas.izqRa, y, 6.7, true);
      ...
      escribir(numero(der ? Number(der.tValue) - Number(der.hValue) : null), columnas.derRa, y, 6.7, true);
      escribir(der?.observacion ?? izq?.observacion, 548, y, 5.2);
    }

    if (ficha.todasConformes !== null) escribir('X', ficha.todasConformes ? 390 : 438, 193, 8, true);
    ...
    const comentarios = (ficha.comentariosActividad ?? '').match(/.{1,115}(?:\s|$)/g) ?? [];
    comentarios.slice(0, 5).forEach((linea, indice) => escribir(linea.trim(), 45, 115 - indice * 9, 5.7));
