...
const PLANTILLA =
  '/UT-UF-MTO-FR-215 CARTILLA DE MANTENIMIENTO PREVENTIVO IM1 - TREN ALSTOM v6.pdf'
const COCHES = ['MA1', 'MB1', 'MB3', 'REM', 'MB2', 'MA2']
const FILAS_Y = [
  710, 697, 684, 671, 644, 630, 617, 604, 577, 564, 551, 537, 511, 497, 471,
  457, 430, 417, 404, 390, 364, 350, 337, 324,
]
const COORDENADA_NUMERO_COCHE: Partial<
  Record<string, { x: number; y: number }>
> = {
  REM: { x: 255, y: 476 },
}
const CODIGO_BOGIE_X = 39
// La serie manuscrita va levemente por encima del subrayado de cada celda
// "Bogie / Código", no pegada al rótulo impreso PB/TB.
const CODIGO_BOGIE_Y_OFFSET = -15
const MAXIMO_ALTO_FIRMA_PERSONA = 11
...
function descargar(bytes: Uint8Array, nombre: string) {
  ...
  const url = URL.createObjectURL(
    new Blob([copia.buffer], { type: 'application/pdf' }),
  )
...
function dividirEnLineas(
  fuente: Awaited<ReturnType<PDFDocument['embedFont']>>,
  valor: string,
  size: number,
  anchoMaximo: number,
) {
...
function truncarConElipsis(
  ...
  while (
    recorte.length > 0 &&
    fuente.widthOfTextAtSize(`${recorte}…`, size) > anchoMaximo
  ) {
...
export async function descargarCartillaPdf(
  ficha: FichaMedicion,
  ...
  const bytes = await fetch(PLANTILLA).then(async (respuesta) => {
    if (!respuesta.ok)
      throw new Error('No se pudo cargar la plantilla de la cartilla.')
  ...
  const escribir = (
    ...
    if (valor)
      page.drawText(valor, { x, y, size, font: fuente, color: rgb(0, 0, 0) })
  }
  const escribirCentrado = (
    ...
    escribir(
      page,
      valor,
      centroX - fuente.widthOfTextAtSize(valor, size) / 2,
      y,
      size,
    )
  }
  ...
  // Misma caja visual de firma para técnicos, Ing. MR y Responsable de
  // Mantenimiento; el área más restrictiva es Técnico 1/2, así que se usa ese
  // alto común para que ninguna firma quede más grande que otra.
  const posiciones = [
  ...
  for (const [indiceCoche, coche] of COCHES.entries()) {
    ...
    const numeroCoche = filasPorPosicion.get(
      `${coche}|${indiceCoche * 4 + 1}`,
    )?.numeroCoche
    if (numeroCoche !== null && numeroCoche !== undefined) {
      const coordenadaNumero = COORDENADA_NUMERO_COCHE[coche] ?? {
        x: 255,
        y: FILAS_Y[indiceCoche * 4 + 2],
      }
      escribirCentrado(
        page,
        String(numeroCoche),
    ...
    for (const ejeInicio of [1, 3]) {
      const fila = filasPorPosicion.get(
        `${coche}|${indiceCoche * 4 + ejeInicio}`,
      )
      const codigo = fila
      ...
      if (codigo) {
        escribirCentrado(
          ...
          serieBogie(codigo),
          CODIGO_BOGIE_X,
          FILAS_Y[indiceCoche * 4 + ejeInicio - 1] + CODIGO_BOGIE_Y_OFFSET,
          5,
        )
      }
    }
  ...
  if (ficha.todasConformes !== null)
    escribir(page, 'X', ficha.todasConformes ? 339.47 : 361, 253, 8)
  ...
  if (comentario) {
    ...
    if (lineas.length > maximoLineas) {
      ...
      lineasVisibles[ultimoIndice] = truncarConElipsis(
        fuente,
        lineasVisibles[ultimoIndice],
        tamanoObs,
        anchoMaximoObs,
      )
    }
    lineasVisibles.forEach((linea, indice) => {
      escribir(
        page,
        linea,
        indice === 0 ? 70 : 60, // primera línea inline tras la etiqueta, resto alineado al margen de la caja
        yObservaciones - indice * interlineado,
        tamanoObs,
      )
    })
  }
  ...
  for (const tecnico of ficha.tecnicos) {
    ...
    await dibujarFirma(
      ...
      posicion.firma[1],
      undefined,
      MAXIMO_ALTO_FIRMA_PERSONA,
    )
    escribir(
      page,
      texto(tecnico.nombre),
      posicion.nombre[0],
      posicion.nombre[1],
      4.5,
    )
  ...
  descargar(
    await pdf.save(),
    `cartilla-preventiva-tren-${ficha.trenNumero}-${ficha.fechaFicha.slice(0, 10)}.pdf`,
  )
}
