...
// Mismo criterio SVG-a-mano que GraficoFlujoMensualDiscos.tsx, pero fijo al
// año calendario (no ventana rodante) y con las 3 series SIEMPRE pobladas
// (nunca null) — ver apps/web/src/features/dashboard/lib/consumoDiscos.ts
// para la transformación de datos y la geometría, compartidas con
// GraficoConsumoDiscosAcumulado.tsx.
type Props = {
...
export function GraficoConsumoDiscos({ retirados, reales, proyeccion, cargando }: Props) {
  const [activo, setActivo] = useState<number | null>(null)
  const contenedorRef = useRef<HTMLDivElement>(null)
  const datos = useMemo(
    () => combinarConsumo(retirados ?? [], reales ?? [], proyeccion ?? []),
    ...
    <GlassSurface fuerte className="rounded-glass p-5">
      <div className="mb-0.5 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          <Activity size={16} className="text-concreto-oscuro" aria-hidden />
          <h3 className="font-display text-base font-semibold text-concreto-oscuro">Consumo de discos</h3>
          <WarningTooltip texto="Discos proyectados para cambiar vs. cambios reales ejecutados vs. discos retirados de Almacén a Taller. Solo el año en curso.">
            <Info size={14} className="text-concreto" aria-label="Más información" />
          </WarningTooltip>
        </div>
        <BotonDescargarGrafico objetivoRef={contenedorRef} nombreArchivo={`consumo-discos-${new Date().getFullYear()}.png`} />
      </div>
      <p className="mb-3 font-body text-xs text-concreto">Enero a diciembre, {new Date().getFullYear()}</p>
      ...
      {cargando ? (
        ...
        <div ref={contenedorRef} className="h-[300px] w-full overflow-x-auto bg-white">
          <div className="relative h-full min-w-[46rem]">
            <svg viewBox={`0 0 ${ANCHO_CONSUMO} ${ALTO_CONSUMO}`} preserveAspectRatio="none" className="h-full w-full" role="img" aria-label="Consumo de discos">
              {ticks.map((t) => (
                <line key={t.valor} x1={MARGEN_CONSUMO.left} x2={ANCHO_CONSUMO - MARGEN_CONSUMO.right} y1={t.y} y2={t.y} stroke="rgba(140,137,127,0.18)" />
              ))}
              ...
              {datos.map((d, i) => (
                <rect
                  ...
                  y={MARGEN_CONSUMO.top}
                  width={ANCHO_CONSUMO / Math.max(datos.length, 1)}
                  height={ALTO_CONSUMO - MARGEN_CONSUMO.top - MARGEN_CONSUMO.bottom}
                  fill="transparent"
            ...
            <div className="pointer-events-none absolute inset-0">
              {ticks.map((t) => (
                <span
                  ...
                  style={{ left: `${pctX(MARGEN_CONSUMO.left - 8)}%`, top: `${pctY(t.y)}%`, transform: 'translate(-100%, -50%)', color: 'var(--color-gris-concreto)' }}
                >
                  {t.valor}
                </span>
              ))}
              ...
              {datos.map((d, i) => (
                <span
                  ...
                  style={{ left: `${pctX(xs[i])}%`, top: `${pctY(ALTO_CONSUMO - MARGEN_CONSUMO.bottom + 18)}%`, transform: 'translate(-50%, -50%)', color: 'var(--color-gris-concreto)' }}
                >
            ...
            {activo !== null && datos[activo] && <TooltipMesConsumo punto={datos[activo]} x={xs[activo]} />}
          </div>
        </div>
      )}
    </GlassSurface>
...
function TooltipMesConsumo({ punto, x }: { punto: PuntoConsumo; x: number }) {
