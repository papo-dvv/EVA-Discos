...
export function GlassModal({ titulo, onCerrar, ancho = 420, children }: GlassModalProps) {
  ...
  return createPortal(
    ...
    >
      <GlassSurface
        ...
        className="rounded-glass-lg p-6"
        style={{
          width: ancho,
          maxWidth: '100%',
          opacity: visible ? 1 : 0,
      ...
      >
        {titulo && (
          <h2 className="mb-1 font-display text-lg font-semibold text-concreto-oscuro">{titulo}</h2>
        )}
        {children}
      </GlassSurface>
    </div>,
